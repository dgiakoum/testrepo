...
char* label_file = "/root/gmail_label\0"; /* This is the settings file */
...
String file_read( char* filename )
{
  String responce="";
  char c;
  File file_handle = FileSystem.open(filename, FILE_READ);
  while (file_handle.available() > 0){
...
*/
 ...
 Serial.println("!2");
  ...
  /*
  username=file_read(login_file);
  password=file_read(passwd_file);
  label=file_read(label_file);
